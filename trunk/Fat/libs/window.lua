...
Window.new = function(_title, _width, _height)
    ...
    local draw = function(self)
        ...
        screen.drawFillRect(SCREEN_DOWN, x + borderWidth, y + borderWidth,
                            x + width - borderWidth, y + height - borderWidth, backgroundColor)
        screen.drawGradientRect(SCREEN_DOWN, x + borderWidth, y + borderWidth,
                                x + width - borderWidth, y + 10 - borderWidth,
                                gradientColor1, gradientColor2, gradientColor1, gradientColor2)
        if closable then
            screen.drawFillRect(SCREEN_DOWN, x + width - borderWidth - 8, y + borderWidth,
                                x + width - borderWidth, y + borderWidth + 8, closeColor)
            screen.drawLine(SCREEN_DOWN, x + width - borderWidth - 7, y + borderWidth + 1,
                            x + width - borderWidth - 1, y + borderWidth + 7, Color.new(31, 31, 31))
            screen.drawLine(SCREEN_DOWN, x + width - borderWidth - 1, y + borderWidth + 1,
                            x + width - borderWidth - 7, y + borderWidth + 7, Color.new(31, 31, 31))
        end
        if icon == nil then
            screen.print(SCREEN_DOWN, x + borderWidth + 2, y + borderWidth,
                         title, titleColor)
        else
            assert(Image.width(icon) ~= 8 and
                   Image.height(icon) ~= 8, "Logo size must be 8x8 pixels max")
            screen.blit(SCREEN_DOWN, x + borderWidth, y + borderWidth, icon)
            screen.print(SCREEN_DOWN, x + borderWidth + 10, y + borderWidth,
                         title, titleColor)
        end
    ...
    local draw = function(self, baseX, baseY)
        ...
        if font == nil then
            if backgroundColor ~= nil then
                screen.drawFillRect(SCREEN_DOWN, baseX + x, baseY + y,
                                    baseX + x + (6 * text:len()), baseY + y + 8,
                                    backgroundColor)
            end
            ...
            if backgroundColor ~= nil then
                screen.drawFillRect(SCREEN_DOWN, baseX + x, baseY + y,
                                    baseX + x + Font.getStringWidth(font, text), baseY + y + Font.getCharHeight(font),
                                    backgroundColor)
            end
