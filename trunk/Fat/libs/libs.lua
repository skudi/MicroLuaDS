...
stopDrawing = function()
    ...
    if Debug.isDebugOn then
        screen.drawTextBox(SCREEN_DOWN, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                           Debug.debugText, Debug.debugColor)
    end
    screen.endDrawing()
    screen.waitForVBL()
end
...
Sprite = {
    ...
    new = function(path_, width_, height_, dest)
        ...
        local drawFrame = function(self, scr, x, y, noFrame)
            assert(scr == SCREEN_UP or
                   scr == SCREEN_DOWN or
                   scr == SCREEN_BOTH, "Bad screen number")
        ...
        local isAnimationAtEnd = function(self, noAnim)
            assert(noAnim > 0, "Animation number must be 1 or more")
            return math.floor(animations[noAnim].tmr:time()/animations[noAnim].delay + 1) >=
                   table.getn(animations[noAnim].tabAnim)
        end
        ...
        local playAnimation = function(self, scr, x, y, noAnim)
            ...
            if math.floor(animations[noAnim].tmr:time()/animations[noAnim].delay) >=
               table.getn(animations[noAnim].tabAnim) then
